...
(function() {
  ...
  function setupAuthenticationSync() {
    ...
    authService.onAuthStateChanged(async (firebaseUser) => {
      if (firebaseUser) {
        ...
        // Set up presence
        authService.setupPresence(firebaseUser.uid);
  ...
  function displayFirebaseMessage(message, fromHistory = false) {
    ...
    if (!chatMessages) {
      console.error('❌ Firebase Bridge: chatMessages element not found!');
      return;
    }
    ...
    const isSameUser = lastMessage && 
                      lastMessage.querySelector('.message-username')?.textContent === message.senderUsername;

    let messageHTML;
    const messageClass = isSent ? 'sent' : 'received';
    const avatarIndex = isSent ? 4 : 1;
    
    if (isSameUser) {
      // Same user, just add text
      messageHTML = `
      ...
      // New user or different sender
      const avatarSeed = message.senderUsername.split('').reduce((acc, c) => acc + c.charCodeAt(0), 0) % 70;
      const defaultAvatar = `https://api.dicebear.com/7.x/thumbs/svg?seed=${avatarSeed}`;
      
      messageHTML = `
        <div class="message ${messageClass}">
          <img class="message-avatar firebase-message-avatar" data-username="${message.senderUsername}" src="${defaultAvatar}" alt="Avatar">
    ...
    console.log('✅ Firebase Bridge: Inserting message HTML');
    chatMessages.insertAdjacentHTML('beforeend', messageHTML);
    
    // Load profile picture from Firebase if available
    if (!isSameUser && typeof window.getFirebaseProfile === 'function') {
      // Check cache first
      const cachedProfile = window.profileDataCache && window.profileDataCache[message.senderUsername];
      if (cachedProfile && cachedProfile.profilePicture) {
        // Use cached profile picture immediately
        const avatarImg = chatMessages.querySelector(`.firebase-message-avatar[data-username="${message.senderUsername}"]:last-of-type`);
        if (avatarImg) {
          avatarImg.src = cachedProfile.profilePicture;
          console.log('✅ Firebase Bridge: Used cached avatar for', message.senderUsername);
        }
      } else {
        // Load from Firebase and cache
        window.getFirebaseProfile(message.senderUsername).then(profile => {
          if (profile && profile.profilePicture) {
            // Cache it
            window.profileDataCache = window.profileDataCache || {};
            window.profileDataCache[message.senderUsername] = profile;
            
            // Update all avatars for this user
            const avatarImgs = chatMessages.querySelectorAll(`.firebase-message-avatar[data-username="${message.senderUsername}"]`);
            avatarImgs.forEach(img => {
              img.src = profile.profilePicture;
            });
            console.log('✅ Firebase Bridge: Updated avatar for', message.senderUsername);
          }
    ...
    console.log('✅ Firebase Bridge: Message displayed successfully');
  }
