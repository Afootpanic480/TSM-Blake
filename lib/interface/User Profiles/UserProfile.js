...
(function() {
  ...
  // Fetch user bio from Firebase (faster and more reliable)
    ...
    try {
      ...
      if (typeof window.getFirebaseProfile === 'function') {
        ...
        if (profile && profile.bio !== undefined) {
          console.log('Bio fetched from Firebase:', profile.bio);
          return profile.bio;
        }
      }
      
      // Fallback: check if user data is cached locally
      if (window.currentUserData && window.currentUserData.username === username) {
        console.log('Bio fetched from local cache:', window.currentUserData.bio);
        return window.currentUserData.bio || '';
      }
  ...
  // Save user profile to Firebase (bio, displayName, bannerColor)
  async function saveUserBio(uuid, bio, displayName = null, bannerColor = null) {
    try {
      if (!window.currentUserData || !window.currentUserData.firebaseUid) {
        throw new Error('Firebase UID not available. Please log in again.');
      }
      ...
      console.log('Saving profile to Firebase with UID:', window.currentUserData.firebaseUid);
      ...
      // Add bannerColor if provided
      if (bannerColor !== null) {
        updates.bannerColor = bannerColor;
        window.currentUserData.bannerColor = bannerColor;
      }
      
      await window.firebaseServices.auth.updateUserProfile(window.currentUserData.firebaseUid, updates);
      
      // Update local cache
      window.currentUserData.bio = bio;
      
      console.log('Profile saved to Firebase successfully:', updates);
      return true;
    } catch (error) {
  ...
  window.showUserProfile = function({ username, avatarUrl, badge, email, bio }) {
    ...
    async function updateProfileData() {
      ...
      // Function to update UI with permissions
      const updatePermissionUI = (userPermission) => {
        let perms = [];
      ...
      // OPTIMISTIC: Show cached permission immediately if available
      const cachedPermission = window.userPermissionsCache && window.userPermissionsCache[username];
      if (cachedPermission || badge) {
        console.log('✅ Using cached permission:', cachedPermission || badge);
        updatePermissionUI(cachedPermission || badge);
      }
      
      // BACKGROUND: Fetch fresh permission and update if different
      if (typeof fetchUserPermission === 'function') {
        fetchUserPermission(username).then(freshPermission => {
          if (freshPermission && freshPermission !== cachedPermission) {
            console.log('🔄 Permission updated:', freshPermission);
            updatePermissionUI(freshPermission);
          }
        }).catch(err => console.warn('Could not fetch fresh permission:', err));
      }
      
      // Load ALL profile data from Firebase (optimistic + background)
      if (typeof window.getFirebaseProfile === 'function') {
        // OPTIMISTIC: Use cached profile data if available
        const cachedProfile = window.profileDataCache && window.profileDataCache[username];
        if (cachedProfile) {
          console.log('✅ Using cached profile data');
          bioDiv.textContent = cachedProfile.bio || 'No bio set';
          if (cachedProfile.displayName && cachedProfile.displayName !== username) {
            const displayNameEl = content.querySelector('.profile-username');
            if (displayNameEl) {
              displayNameEl.textContent = cachedProfile.displayName;
              displayNameEl.title = `@${username}`;
            }
          }
          const bannerColor = cachedProfile.bannerColor || '#5865f2';
          content.style.setProperty('--accent', bannerColor);
          content.style.setProperty('--banner-color', bannerColor);
          if (cachedProfile.profilePicture) {
            const avatarImg = content.querySelector('.profile-avatar');
            if (avatarImg) avatarImg.src = cachedProfile.profilePicture;
          }
        }
        
        // BACKGROUND: Fetch fresh profile data
        try {
          const profile = await window.getFirebaseProfile(username);
          if (profile) {
            ...
            bioDiv.textContent = profile.bio || 'No bio set';
            ...
            const bannerColor = profile.bannerColor || '#5865f2';
            content.style.setProperty('--accent', bannerColor);
            content.style.setProperty('--banner-color', bannerColor);
            
            if (profile.profilePicture) {
              const avatarImg = content.querySelector('.profile-avatar');
              if (avatarImg) avatarImg.src = profile.profilePicture;
            }
            
            console.log('🔄 Profile data updated from Firebase:', profile);
          } else if (!cachedProfile) {
            bioDiv.textContent = 'No bio set';
            content.style.setProperty('--accent', '#7289da');
          }
        } catch (e) {
