...
document.addEventListener('DOMContentLoaded', function() {
  // Setup delegated context menus once CustomContextMenu is available
  (function setupDelegatedMenus() {
    function registerAll() {
      if (typeof registerContextMenuDelegate !== 'function') {
        setTimeout(registerAll, 100);
        return;
      }
      const tSafe = (k, d) => (typeof t === 'function' ? t(k) : d);
      const labelAddFriend = tSafe('addFriend', 'Add Friend');
      const labelPending = tSafe('friendRequests', 'Friend Requests');
      const labelDM = 'Direct Messages';
      const labelProfile = 'Profile';

      // Shared chat menus
      const chatMenu = [
        { label: 'Chat', type: 'header' },
      ...
      registerContextMenuDelegate('.chat-messages', chatMenu);
      registerContextMenuDelegate('.chat-home-view', chatMenu);

      // DM bar
      const dmMenu = [
        { label: labelDM, type: 'header' },
        { label: labelAddFriend, icon: '➕', onClick: () => { if (typeof showFriendsView === 'function') showFriendsView('add'); } },
        { label: 'Mark All Read', icon: '✅', onClick: () => showAlert('All messages have been marked as read', 'success') },
      ];
      registerContextMenuDelegate('.sidebar-nav', dmMenu);

      // Sidebar footer
      const footerMenu = [
        { label: labelProfile, type: 'header' },
      ...
      registerContextMenuDelegate('.sidebar-footer', footerMenu);
      registerContextMenuDelegate('.sidebar-footer-inner', footerMenu);
    }
    registerAll();
  })();
});
