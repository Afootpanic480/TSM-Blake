...
async function handleLoginWithLogin() {
  ...
  if (!document.getElementById(styleId)) {
    ...
    style.textContent = `
      ...
      .login-dot:nth-child(1) { animation: pulse 1.8s ease-in-out infinite 0s; }
      .login-dot:nth-child(2) { animation: pulse 1.8s ease-in-out infinite 0.6s; }
      .login-dot:nth-child(3) { animation: pulse 1.8s ease-in-out infinite 1.2s; }
      .login-message {
        color: #b9bbbe;
        margin-top: 15px;
        font-size: 1.2em;
  ...
  if (!loadingContainer) {
    ...
    `;
    // Insert after the app-logo (title) inside password-box
    const passwordBox = document.querySelector('.password-box');
    const appLogo = passwordBox ? passwordBox.querySelector('.app-logo') : null;
    if (passwordBox && appLogo) {
      // Insert right after the logo/title
      appLogo.insertAdjacentElement('afterend', loadingContainer);
    } else if (passwordBox) {
      passwordBox.appendChild(loadingContainer);
    } else if (loginForm) {
